...
int main(int argc, char *argv[]) {
  ...
  /* je passe en paramètre l'adresse de la socket du serveur (IP et
     numéro de port) si besoin est,  je modifie pour mettre  plus de paramètres */
  ...
  /* Etape 1 : créer une socket Cient*/   
  int SocketClientUDP = socket(AF_INET,SOCK_DGRAM,0);//TODO
  ...
  printf("Client : Début du TP noté\n");
...
//ETAPE 2 :
 ...
 AdrCLientTP.sin_port = htons(42540);//numero du port

//bind fonction qui relie une socket a une adress
if(bind(SocketClientUDP,(struct sockaddr*)&AdrCLientTP,sizeof(AdrCLientTP))==-1){
  ...
  if(sendto(SocketClientUDP, msg, taillemsg, 0, (struct sockaddr*)&AdrSErveurTP, sizeof(struct sockaddr)) == -1){
  ...
  if (recvfrom(SocketClientUDP, buffer, tailleBuffer, 0, (struct sockaddr*)&AdrSErveurTP, &tailleAdresse) == -1) {
